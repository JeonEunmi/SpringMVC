...
<script>
	$(document).ready(function() {
		...
		$(".btnUp").on("click", function() {
			...
			var name_ = $(siblings).eq(1).text();
			var phone = $(siblings).eq(2).text();
			var email = $(siblings).eq(3).text();
			var regDate = $(siblings).eq(4).text();
				...
				<form action="${pageContext.request.contextPath}/member/insert" method="post">
					...
					<div class="form-group">
						<input type="text" class="form-control" id="email" name="email"
							placeholder="Email(max 40)" required value="">
					</div>
					<div class="form-group">
						<input type="date" class="form-control" id="regDate" name="regDate"
							placeholder="RegDate" required value="">
					</div>
